...
func ConversationList(c *gin.Context) {
	...
	err = requestParams.DisposeRequest(engine.Table(ConversationTableName)).Find(&tableEntities)
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
	count, err := requestParams.DisposeRequest(engine.Table(ConversationTableName)).Count()
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
...
func ConversationAdd(c *gin.Context) {
	var table models.Conversation
...
func ConversationUpdate(c *gin.Context) {
	var table models.Conversation
	err := c.ShouldBindJSON(&table)
	if err != nil {
		common.ResultError(500, validator.Translate(err), c)
...
func ConversationGetOne(c *gin.Context) {
	var table models.Conversation
	id := c.Param("id")
	if id == "" {
		common.ResultError(500, "Id不能为空", c)
