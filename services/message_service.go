...
func MessageList(c *gin.Context) {
	...
	requestParams.Query["sender_i_d"] = common.GetCurrentUser(c)
	var tableEntities []models.Message
	err = requestParams.DisposeRequest(engine.Table(MessageTableName)).Find(&tableEntities)
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
	count, err := requestParams.DisposeRequest(engine.Table(MessageTableName)).Count()
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
...
func MessageGetOne(c *gin.Context) {
	var table models.Message
	id := c.Param("id")
	if id == "" {
		common.ResultError(500, "Id不能为空", c)
