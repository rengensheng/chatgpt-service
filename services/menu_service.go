...
func MenuList(c *gin.Context) {
	...
	var tableEntities []models.Menu
	err = requestParams.DisposeRequest(engine.Table(MenuTableName)).Find(&tableEntities)
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
	count, err := requestParams.DisposeRequest(engine.Table(MenuTableName)).Count()
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
