...
func BalanceList(c *gin.Context) {
	...
	var tableEntities []models.Balance
	err = requestParams.DisposeRequest(engine.Table(BalanceTableName)).Find(&tableEntities)
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
	count, err := requestParams.DisposeRequest(engine.Table(BalanceTableName)).Count()
	if err != nil {
		common.ResultError(500, err.Error(), c)
		return
	}
...
func BalanceUpdate(c *gin.Context) {
	var table models.Balance
	err := c.ShouldBindJSON(&table)
	if err != nil {
		common.ResultError(500, validator.Translate(err), c)
...
func BalanceDelete(c *gin.Context) {
	var table models.Balance
	id := c.Param("id")
	if id == "" {
		common.ResultError(500, "Id不能为空", c)
...
func BalanceGetOne(c *gin.Context) {
	var table models.Balance
	id := c.Param("id")
	if id == "" {
		common.ResultError(500, "Id不能为空", c)
